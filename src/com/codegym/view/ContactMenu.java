...
public class ContactMenu {
    ...
    private void updateContact(ContactManagement contactManagement) {
        System.out.println("---Cập nhật thông tin số điên thaoị");
        System.out.println("Nhập số điện thoại bạn muốn thay đổi");
        String phonenumber = scanner.nextLine();
        int index = contactManagement.findIndexByPhonenumber(phonenumber);
        if (index == -1) {
            System.out.println("Không tìm được danh bạ với số điện thoại trên");
        } else {
            Contact contact = inputNewContactInfo();
            contactManagement.updateContact(contact, phonenumber);
    ...
    private void deleteContact(ContactManagement contactManagement) {
        ...
        private void findContact(ContactManagement contactManagement){
            ...
            String name = scanner.nextLine();
            contactManagement.findContactByName(name);
            if(contactManagement.findContactByName(name) == null){
